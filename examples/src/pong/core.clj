(ns pong.core
  "The rules of pong, with no window, no keyboard and no clock.

  The court is W x H cells. A ball is [x y dx dy], a position and a velocity
  in cells per tick. A game is [phase ball left-y right-y left-score
  right-score]; a paddle is PH cells tall and its y is its top row. The
  phase is tagged data: [:Serving n] counts down to a serve, [:Playing],
  [:Paused], or [:Won side].

  pong.main owns the effects: it reads the keyboard into a key, calls `step`
  once a tick, and draws what comes back.")

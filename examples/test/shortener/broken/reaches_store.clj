(ns shortener.broken.reaches-store
  "shortener.core with `shorten` saving the new link to shortener.store
  itself. writ lets a call into another namespace through its purity
  check, the laws never look at the store, and so only the call graph
  shows the core reaching into the shell."

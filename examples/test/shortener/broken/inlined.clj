(ns shortener.broken.inlined
  "shortener.core with `shorten` pasted into `handle`. It behaves the same
  today, and every law holds. The next change to `shorten` will not reach
  the copy."
...
(defn handle [links method path body]
  (let [r (route method path)]
    (case (first r)
      ...
      :Shorten (let [url (normalize-url body)]
                 (cond
                   (not (valid-url? url)) [links [:Text 400 "not an http(s) URL\n"]]
                   (code-for links url) [links [:Created (code-for links url)]]
                   :else (let [code (encode-id (count links))]

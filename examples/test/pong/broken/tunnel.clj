(ns pong.broken.tunnel
  "pong.core with the classic tunnelling bug: the paddle is hit only when the
  ball lands exactly on its column, so a fast ball jumps it.")
...
(defn advance
  ...
  (let [[x y dx dy] ball
        ...
        x1 (+ x dx)]
    (cond
      (and (neg? dx) (= x1 LEFT-X) (covers? ly y2))
